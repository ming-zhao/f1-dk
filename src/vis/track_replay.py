...
callable and testable:
    ...
    write_outputs    JS payload + the player's index (+ optional standalone HTML)
...
Payloads land in data/replay/<year>/<location>.js; open dashboard/replay.html
directly to browse them — no server, because a <script> tag loads from file://
where fetch() cannot.
...
def parse_args(argv: list | None = None) -> argparse.Namespace:
    ...
    ap.add_argument("--standalone", action="store_true",
                    help="also write a self-contained dashboard/replay_<year>_<loc>.html "
                         "with the payload inlined (~3 MB). Only for emailing ONE race as "
                         "a single file: dashboard/replay.html already opens directly "
                         "off disk and shows every race in data/replay/, at ~50 KB")
...
def _read_payload(f: Path) -> dict:
    """Parse a payload written by write_outputs (a JS assignment, not bare JSON)."""
    text = f.read_text(encoding="utf-8")
    return json.loads(text[text.index("=") + 1:].rstrip().rstrip(";"))


def refresh_index(data_dir: Path) -> list:
    """Rebuild index.js from every payload on disk, across all years.
    ...
    path (`2024/Monaco.js`) that the picker can load directly.

    Both the index and the payloads are written as **JavaScript**, not JSON: they are
    loaded with a <script> tag rather than fetch(), which is what lets
    dashboard/replay.html work when opened straight off disk. fetch() is blocked on a
    file:// origin (opaque origin, CORS denies it); script tags are exempt. That is
    exactly why dashboard/index.html has always worked by double-clicking while the
    replay picker did not.
    """
    index = []
    ...
    slim = [{k: r[k] for k in ("year", "round", "location", "fromLap", "laps",
                               "running", "retired")} | {"file": r["_file"]}
            for r in index]
    # Left over from when the index was fetched as JSON; nothing reads it now.
    stale_index = data_dir / "index.json"
    if stale_index.exists():
        stale_index.unlink()
    (data_dir / "index.js").write_text(
        "// Generated by src/vis/track_replay.py — the list of built replays.\n"
        "window.REPLAY_INDEX = " + json.dumps(slim, indent=1) + ";\n",
        encoding="utf-8")
    return index
...
def write_outputs(session: dict, opts: argparse.Namespace, html: str,
    ...
    # Location only: the year is already the directory name. Written as a JS
    # assignment rather than bare JSON so the picker can <script>-load it from
    # file:// — see refresh_index for why.
    jf = year_dir / f"{session.get('location', 'race')}.js"
    jf.write_text("window.REPLAY_RACE = "
                  + json.dumps(payload, separators=(",", ":")) + ";\n",
                  encoding="utf-8")
    # A payload left over from when these were .json would shadow the new one in the
    # index, so clear it.
    stale = jf.with_suffix(".json")
    if stale.exists():
        stale.unlink()
    ...
    print(f"Wrote {jf.relative_to(SRC.parent)} "
          ...
          f"{(REPLAY_DIR / 'index.js').relative_to(SRC.parent)} "
          f"({len(index)} race(s)) — open dashboard/replay.html directly.")
