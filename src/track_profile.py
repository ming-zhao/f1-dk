...
Usage:
    python3 src/track_profile.py                 # all circuits, recent era
    python3 src/track_profile.py --since 2014    # custom era cutoff
    python3 src/track_profile.py --circuit spa   # one circuit, with per-race detail
"""
...
def main():
    ...
    if df.empty:
        print(f"No results at or after {since}. Run src/fetch_jolpica.py first.")
        return
    df = annotate(df)
