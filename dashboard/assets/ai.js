...
function renderAiResults() {
  ...
  const phaseNote = AI.running
    ? (fitCount < AI_OPTIMIZE_MIN_FIT
        ? ` Filling: ${fitCount}/${aiCandidates.length} lineups fit the description (locked in) — need ${AI_OPTIMIZE_MIN_FIT} before optimizing.`
        : ` ${fitCount}/${aiCandidates.length} lineups fit the description (${AI_OPTIMIZE_MIN_FIT}+) — now upgrading the weakest toward the best.`)
    : "";
...
function updateAiChances(simsPerCandidate) {
  ...
  for (let i = 0; i < AI.shortlist.length; i++) {
    ...
    // LOCK — the real fix for "you throw away lineups that work". Once a slot's
    // lineup is CONFIRMED to fit the description, it is locked for the rest of the
    // run: we keep it and just keep running sims on it, and never re-evaluate it
    // out of existence. This is sound because the Min-pick-chance bar can't change
    // mid-run (its dropdown is disabled while running) and a lineup's TRUE fit
    // status is fixed — only our noisy chances estimate wobbles. Without this, a
    // fitting lineup whose weakest pick sits near the bar keeps failing a later
    // noisy re-check by chance and gets discarded, which is exactly the bug.
    // (User rule: below 10 fitting lineups, keep simming the ones that fit — don't
    // throw them away. See doc/ai-simulation-requirements.md.)
    ...
    // The 300-sim light pass is noisy, so never trust a flip on it alone. Re-verify
    // with a big, reliable pass whenever the light read says this slot fits (so a
    // real fit gets confirmed and LOCKED), OR when it reads sub-bar but the slot fit
    // last pass (so a fitting lineup is never dropped on a lone flicker before it
    // has locked). A confirmed fit locks the slot in for the run.
    if (fits || prevFit[i]) {
    ...
    if (fits) AI.fitLocked[i] = true; // confirmed fitting — lock it for the rest of the run
    AI.visible[i] = fits;
  }
