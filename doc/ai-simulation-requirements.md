...
- **When a lineup fits the requirement, don't change it — just keep it there —
  until MORE THAN 10 lineups fit the requirement.** A lineup that already fits is
  never deleted or swapped out while 10 or fewer fit; it stays put and keeps
  being simulated. Only the slots that *don't* yet fit are worked on during this
  phase.
- **Once more than 10 lineups fit, changing is allowed.** From that point the
  search may start upgrading the weakest fitting slots toward the best (pulling
  better candidates in). If enough fitting lineups later fall below the bar so
  that 10 or fewer fit again, upgrading pauses and the search goes back to just
  filling.
- *(Code: `performAiSwaps()` gates its upgrade path on
  `canOptimize = fitCount > AI_OPTIMIZE_MIN_FIT`, where `AI_OPTIMIZE_MIN_FIT = 10`.)*
- **A fitting lineup must not disappear on measurement noise.** Whether a lineup
  fits is read from a light 300-sim chances pass, which is noisy — a lineup that
  genuinely fits can momentarily read one pick below the bar. Never act on that:
  if a lineup fit last pass but this light pass reads it sub-bar, re-verify with a
  much bigger, reliable chances pass before believing it. Only a *confirmed*
  failure is allowed to mark a lineup not-fitting (which is the only thing that
  lets it be repaired/replaced). *(Code: the "flicker guard" in
  `updateAiChances()` — a fit-last-pass slot flagged sub-bar is rechecked at
  `AI_CHAMP_RECHECK_SIMS` before `AI.visible[i]` is set. Previously only the
  champion got this, so other fitting lineups vanished on a single noisy pass.)*
